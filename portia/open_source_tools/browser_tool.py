...
def convert_model_to_browser_use_model(model: GenerativeModel) -> BaseChatModel:  # noqa: C901
    ...
    if validate_extras_dependencies("ollama", raise_error=False):
        ...
        if isinstance(model, OllamaGenerativeModel):
            return ChatOllama(model=model.model_name, host=model.base_url)
    ...
    if validate_extras_dependencies("amazon", raise_error=False):
        ...
        if isinstance(model, AmazonBedrockGenerativeModel):
            auth: dict[str, Any] = {}
            if model.aws_access_key_id and model.aws_secret_access_key:
                auth = {
                    "aws_access_key_id": model.aws_access_key_id,
                    "aws_secret_access_key": model.aws_secret_access_key,
                }
            elif model.credentials_profile_name:
                session = boto3.Session(profile_name=model.credentials_profile_name)
                auth = {
                    "session": session,
                }
            return ChatAWSBedrock(
                model=model.model_name,
                aws_region=model.region_name,
                **auth,
            )
