...
class LogCapture:
    ...
    def __init__(self, log_widget: RichLog) -> None:
        ...
        self.handler = LogHandler(log_widget)
...
class LogHandler(logging.Handler):
    """Custom logging handler that sends logs to the RichLog widget."""
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        try:
            ...
            if hasattr(self.log_widget, 'app') and self.log_widget.app:
                self.log_widget.app.call_from_thread(self._add_log, msg, record.levelname)
        except Exception:
        ...
        try:
            ...
            while execution_thread.is_alive() and not self._stop_requested:
                if self.plan_run and self.plan_run.current_step_index != last_step_index:
                    ...
                    if last_step_index >= 0:
                        self.call_from_thread(plan_panel.update_step_status, last_step_index, "completed")
                    
                    # Mark current step as running
                    if current_step < len(self.plan.steps):
                        self.call_from_thread(plan_panel.update_step_status, current_step, "running")
            ...
            if self.plan_run:
                if self.plan_run.state == PlanRunState.COMPLETE:
                    ...
                    self.call_from_thread(log_widget.write, "[green]Execution completed successfully![/green]")
                    ...
                    if last_step_index >= 0:
                        self.call_from_thread(plan_panel.update_step_status, last_step_index, "failed")
...
if __name__ == "__main__":
    ...
    try:
        from portia import Config, example_tool_registry
        from portia.errors import InvalidConfigError
        
        # Try to create Portia with error handling
        try:
            portia = Portia(
                Config.from_default(),
            ...
            print(f"   {e}")
            print("\n💡 To fix this, you need to set up an API key for an LLM provider:")
            print("   1. Create a .env file with: OPENAI_API_KEY=your_openai_api_key")
            print("   2. Or export: export OPENAI_API_KEY=your_openai_api_key")
            print("   3. Get API keys from:")
            print("      - OpenAI: https://platform.openai.com/api-keys")
            print("      - Anthropic: https://console.anthropic.com/")
            print("      - Google AI: https://makersuite.google.com/app/apikey")
            print("\n🎯 For a better experience, run: python example_gui.py")
